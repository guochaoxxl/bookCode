...
//2个进程，一个进程完成每次加1,另一个进程完成每次加2,2个进程协作完成累加,使用共享内存方式在进程间通信  
  ...
  pthread_mutexattr_setpshared(&mutexattr,PTHREAD_PROCESS_SHARED);//设置互斥对象为PTHREAD_PROCESS_SHARED共享，即可以在多个进程的线程访问,PTHREAD_PROCESS_PRIVATE为同一进程的线程共享  
   ...
   if (rt==0){//子进程完成x+1  
       shm_id=shm_open(addnum,O_RDWR,0);         ptr=mmap(NULL,sizeof(int),PROT_READ|PROT_WRITE,MAP_SHARED,shm_id,0);/*连接共享内存区*/  
       x=(int *)ptr;    
       ...
       shm_id=shm_open(addnum,O_RDWR|O_CREAT,0644);  
        ftruncate(shm_id,sizeof(int));          ptr=mmap(NULL,sizeof(int),PROT_READ|PROT_WRITE,MAP_SHARED,shm_id,0);/*连接共享内存区*/  
        x=(int *)ptr;   
